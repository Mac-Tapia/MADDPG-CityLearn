...
for i, (name, color, grid, solar, battery, hvac, ev) in enumerate(methods_data):
    ...
    ax1.text(3.5, y+0.45, f'Batería\n{battery:.0f}%', ha='center', va='center', fontsize=9, fontweight='bold', color='white')
...
# MADDPG (exterior)
pie_result1 = ax3.pie(maddpg_consumo, radius=1, colors=colors_outer,
                      autopct='%1.0f%%', pctdistance=0.85,
                      wedgeprops=dict(width=size, edgecolor='white'))
# MARLISA (interior)
pie_result2 = ax3.pie(marlisa_consumo, radius=1-size, colors=colors_inner,
                      wedgeprops=dict(width=size, edgecolor='white'))
...
├─────────────────────────────────────────────────────────────────────┤
│  Costo            │  {MADDPG['cost']:.3f}       │  {MARLISA['cost']:.3f}       │  {(MADDPG['cost']-MARLISA['cost'])/MARLISA['cost']*100:+.1f}%          │
│  CO₂              │  {MADDPG['co2']:.3f}       │  {MARLISA['co2']:.3f}       │  {(MADDPG['co2']-MARLISA['co2'])/MARLISA['co2']*100:+.1f}%          │
│  Peak             │  {MADDPG['peak']:.3f}       │  {MARLISA['peak']:.3f}       │  {(MADDPG['peak']-MARLISA['peak'])/MARLISA['peak']*100:+.1f}%          │
│  Consumo          │  {MADDPG['consumption']:.3f}       │  {MARLISA['consumption']:.3f}       │  {(MADDPG['consumption']-MARLISA['consumption'])/MARLISA['consumption']*100:+.1f}%          │
└─────────────────────────────────────────────────────────────────────┘
